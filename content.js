...
class FormFillAssistant {
  ...
  async _saveSettings(s) { try { const existing = await this._getSettings() || {}; await chrome.storage.local.set({ [SETTINGS_KEY]: { ...existing, ...s } }); } catch {} }
  ...
  _fillInputEl(el, content) {
    ...
    // Check if this is a custom widget that needs click instead of value set
    const needsClick = this._needsClickFill(el);
    if (needsClick) {
      this._clickFill(el, content);
      return;
    }
    ...
    // After setting, check if the value actually took (some custom widgets ignore native setter)
    if (el.value !== content && el.value !== '') {
      // Value didn't take — fall back to click
      this._clickFill(el, content);
    }
  }

  // ── 判断是否需要点击填写（自定义下拉 / 选择器组件）─────────────

  _needsClickFill(el) {
    if (el.readOnly) return true;
    const role = el.getAttribute('role');
    if (role === 'combobox' || role === 'listbox') return true;
    // Inputs with dropdown/picker-related classes
    const cls = (el.className || '').toString().toLowerCase();
    if (/(select|dropdown|picker|combobox|autocomplete|chosen)/.test(cls)) return true;
    // Check if parent is a custom select widget
    const parent = el.closest('[class*="select"], [class*="dropdown"], [class*="picker"], [role="combobox"], [role="listbox"]');
    if (parent) {
      const parentCls = (parent.className || '').toString().toLowerCase();
      if (/(select|dropdown|picker|combobox)/.test(parentCls)) return true;
    }
    return false;
  }

  // ── 点击方式填写（下拉框、单选、自定义选择器）────────────────

  _clickFill(el, content) {
    ...
    // Also click parent if it looks like a select trigger
    const parent = el.closest('[class*="select"], [class*="dropdown"], [class*="picker"], [role="combobox"], [role="listbox"]');
    if (parent && parent !== el) {
      parent.click();
    ...
    // Step 2: Wait for popup/dropdown and click the matching option
    this._waitAndClickOption(normContent, el);
  }

  _waitAndClickOption(normalizedValue, triggerEl) {
    ...
    const tryClick = () => {
      const optionSelectors = [
        ...
        '.dropdown-item', '.list-item', '[class*="suggestion"]',
        '[class*="result"]', '.autocomplete-item'
      ];

      for (const sel of optionSelectors) {
        for (const c of document.querySelectorAll(sel)) {
          if (this.sidebar?.contains(c)) continue;
          const text = (c.textContent || '').trim().toLowerCase();
          if (!text) continue;
          // Match: exact, contains, or first char match for short values
    ...
    // Try at intervals (dropdowns often have animation delays)
    const doAttempt = () => {
      if (tryClick()) return;
      attempts++;
      if (attempts < maxAttempts) setTimeout(doAttempt, checkInterval);
    };

    // Initial try after a short delay to let the dropdown render
    setTimeout(doAttempt, 80);
  }
  ...
  async _handleAIFill() {
    if (!OAA_AI.isConfigured()) {
      this._showToast('请先在设置中配置 AI（API Key + Model）', true);
      this._showSettings();
      return;
    }
    if (!this.profile?.orderedData?.length) {
      this._showToast('请先导入数据（同步主页 / 手动编辑 / AI 简历导入）', true);
      return;
    }
    this._setStatus('AI 正在分析表单…', true);

    try {
      const filler = new AIFormFiller(OAA_AI, this.profile, {
        getFieldLabel: (el) => this._getFieldLabel(el),
        fillInputEl: (el, content) => this._fillInputEl(el, content),
        findExactMatch: (label) => this._findExactMatch(label),
        sidebar: this.sidebar
      });
      const result = await filler.fillAll();
      if (result.filled > 0) {
        this._setStatus(`AI 已填写 ${result.filled} 个字段`);
        this._showToast(`AI 智能填表完成：${result.filled} 个字段已填写${result.unmatched > 0 ? `，${result.unmatched} 个字段需手动处理` : ''}`);
      } else {
        this._setStatus('AI 未找到可匹配的字段');
        this._showToast('AI 未找到可匹配的字段，请手动填写', true);
      }
    } catch (err) {
      console.error('[OAA] AI 填表失败:', err);
      this._setStatus('AI 填表失败');
      this._showToast('AI 填表失败：' + err.message, true);
    }
  }

  async _importAIResumeData(aiGeneratedData) {
    if (!aiGeneratedData.orderedData?.length) {
      this._showToast('AI 未生成有效数据', true);
      return;
    }
    const orderedData = aiGeneratedData.orderedData.map((item, i) => ({
      title: String(item.title || '').trim(),
      buttonText: String(item.buttonText || '').trim(),
      fillContent: item.fillContent != null ? String(item.fillContent) : '',
      order: i,
    ...
    if (!orderedData.length) {
      this._showToast('AI 生成的数据中没有有效条目', true);
      return;
    }

    this.profile = {
      name: aiGeneratedData.name || '我',
      orderedData,
      fetchedAt: Date.now(),
      modified: true
    };
    await this._saveData();
    this._setStatus(`AI 导入成功 · ${orderedData.length} 条`);
    this._renderButtons();
    this._showToast(`AI 解析完成，共 ${orderedData.length} 个字段`);
  }

  async _handleResumeFile(file) {
    const ext = file.name.split('.').pop().toLowerCase();
    if (!['pdf', 'docx', 'doc'].includes(ext)) {
      this._showToast('仅支持 PDF 和 Word 文档', true);
      return;
    }
    if (!OAA_AI.isConfigured()) {
      this._showToast('请先在设置中配置 AI（API Key + Model）', true);
    ...
    this._setStatus('正在解析简历…', true);
    try {
      const dialogue = new ResumeDialogue(OAA_AI, this);
      document.getElementById('ffa-settings-btn')?.classList.remove('ffa-icon-active');
      await dialogue.start(file);
    } catch (err) {
      console.error('[OAA] 简历解析失败:', err);
      this._setStatus('简历解析失败');
      this._showToast('简历解析失败：' + err.message, true);
    }
  }
  ...
  _buildSidebar() {
    ...
    statusBar.append(
      ...
      mk('ffa-ai-fill-btn',  '🤖',       'AI 智能填表',       () => this._handleAIFill()),
  ...
  _showSettings() {
    ...
    this._setBodyContent(async c => {
      ...
      // ── AI 配置 ─────────────────────────────────────────────
      const aiLabel = el('p', 'ffa-settings-label', 'AI 配置');
      const aiNote = el('p', 'ffa-settings-note', '支持 OpenAI / Azure / Groq / DeepSeek / Ollama 等兼容接口。简历解析和智能填表均依赖此配置。');

      const endpointInput = document.createElement('input');
      endpointInput.type = 'url'; endpointInput.className = 'ffa-form-input';
      endpointInput.placeholder = 'https://api.openai.com/v1';
      endpointInput.value = settings?.aiEndpoint || '';

      const keyWrap = document.createElement('div'); keyWrap.className = 'ffa-key-wrap';
      const keyInput = document.createElement('input');
      keyInput.type = 'password'; keyInput.className = 'ffa-form-input'; keyInput.placeholder = 'sk-...';
      keyInput.value = settings?.aiKey || '';
      const keyToggle = el('button', 'ffa-key-toggle', '\u{1F441}');
      keyToggle.title = '显示/隐藏';
      keyToggle.addEventListener('click', () => {
        const show = keyInput.type === 'password';
        keyInput.type = show ? 'text' : 'password';
        keyToggle.textContent = show ? '\u{1F648}' : '\u{1F441}';
      });
      keyWrap.append(keyInput, keyToggle);

      const modelInput = document.createElement('input');
      modelInput.type = 'text'; modelInput.className = 'ffa-form-input'; modelInput.placeholder = 'gpt-4o-mini';
      modelInput.value = settings?.aiModel || '';

      const testBtn = el('button', 'ffa-outline-btn ffa-block-btn', '\u{1F517} 测试连接');
      testBtn.addEventListener('click', async () => {
        testBtn.textContent = '测试中…'; testBtn.disabled = true;
        try {
          OAA_AI._endpoint = endpointInput.value.trim() || 'https://api.openai.com/v1';
          OAA_AI._key = keyInput.value.trim();
          OAA_AI._model = modelInput.value.trim() || 'gpt-4o-mini';
          await OAA_AI.testConnection();
          this._showToast('连接成功！');
        } catch (e) {
          this._showToast('连接失败：' + e.message, true);
        } finally {
          testBtn.textContent = '\u{1F517} 测试连接'; testBtn.disabled = false;
        }
      });

      const saveAIBtn = el('button', 'ffa-form-save ffa-block-btn', '保存 AI 配置');
      saveAIBtn.addEventListener('click', async () => {
        const s2 = await this._getSettings() || {};
        s2.aiEndpoint = endpointInput.value.trim() || 'https://api.openai.com/v1';
        s2.aiKey = keyInput.value.trim();
        s2.aiModel = modelInput.value.trim() || 'gpt-4o-mini';
        await this._saveSettings(s2);
        await OAA_AI.init();
        this._showToast('AI 配置已保存');
      });

      // ── AI 简历导入 ─────────────────────────────────────────
      const resumeLabel = el('p', 'ffa-settings-label', 'AI 简历导入');
      const resumeNote = el('p', 'ffa-settings-note', '上传 PDF 或 Word 简历，AI 将与你对话确认后自动生成填写数据。');

      const uploadZone = document.createElement('div'); uploadZone.className = 'ffa-upload-zone';
      const uploadText = el('p', 'ffa-upload-text', '\u{1F4C4} 拖拽简历文件到此处，或点击选择');
      const uploadInput = document.createElement('input');
      uploadInput.type = 'file'; uploadInput.accept = '.pdf,.docx,.doc'; uploadInput.style.display = 'none';
      uploadZone.appendChild(uploadText);
      uploadZone.appendChild(uploadInput);

      uploadZone.addEventListener('click', () => uploadInput.click());
      uploadZone.addEventListener('dragover', e => { e.preventDefault(); uploadZone.classList.add('ffa-upload-dragover'); });
      uploadZone.addEventListener('dragleave', () => uploadZone.classList.remove('ffa-upload-dragover'));
      uploadZone.addEventListener('drop', e => {
        e.preventDefault();
        uploadZone.classList.remove('ffa-upload-dragover');
      ...
      w.append(backBtn, srcLabel, urlInput, updateBtn, urlNote, saveManualBtn, aiLabel, aiNote, endpointInput, keyWrap, modelInput, testBtn, saveAIBtn, resumeLabel, resumeNote, uploadZone, ioLabel, exportBtn, importBtn, importFile, dangerLabel, clearBtn);
      c.appendChild(w);
    });
  }
